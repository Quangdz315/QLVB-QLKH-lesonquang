...
class VanBanDen(models.Model):
    ...
    nhom_van_ban = fields.Selection([
        ('co_quan_doi_tac', 'Từ cơ quan/đối tác'),
        ('nhan_vien', 'Từ nhân viên'),
        ('phong_ban', 'Từ phòng ban'),
        ('noi_bo', 'Văn bản nội bộ'),
        ('chung', 'Văn bản chung'),
    ], string="Nhóm văn bản", default='co_quan_doi_tac', required=True, index=True)

    noi_gui = fields.Char(string="Nơi gửi/Ghi chú nguồn gửi")
    co_quan_doi_tac_gui_id = fields.Many2one(
        'res.partner',
        string="Cơ quan/Đối tác gửi",
    )
    nhan_vien_gui_id = fields.Many2one(
        'nhan_vien',
        string="Nhân viên gửi",
    )
    phong_ban_gui_id = fields.Many2one(
        'phong_ban',
        string="Phòng ban gửi",
    )

    nguoi_nhan = fields.Char(string="Ghi chú người nhận")
    ...
    khach_hang_nhan_ids = fields.Many2many(
        'res.partner',
        'van_ban_den_res_partner_rel',
        'van_ban_den_id',
        'partner_id',
        string="Khách hàng nhận",
    )

    nguon_gui_tong_hop = fields.Char(
        string="Nguồn gửi",
        compute='_compute_nguon_gui_tong_hop',
    )
    ...
    don_vi_nhan = fields.Char(string="Đơn vị nhận xử lý")
    ...
    @api.depends('nhom_van_ban', 'co_quan_doi_tac_gui_id', 'nhan_vien_gui_id', 'phong_ban_gui_id', 'noi_gui')
    def _compute_nguon_gui_tong_hop(self):
        for record in self:
            parts = []
            if record.co_quan_doi_tac_gui_id:
                parts.append('%s (cơ quan/đối tác)' % record.co_quan_doi_tac_gui_id.display_name)
            if record.nhan_vien_gui_id:
                parts.append('%s (nhân viên)' % record.nhan_vien_gui_id.display_name)
            if record.phong_ban_gui_id:
                parts.append('%s (phòng ban)' % record.phong_ban_gui_id.display_name)
            if record.nhom_van_ban == 'noi_bo' and not parts:
                parts.append('Nội bộ')
            if record.nhom_van_ban == 'chung' and not parts:
                parts.append('Văn bản chung')
            if record.noi_gui:
                parts.append(record.noi_gui)
            record.nguon_gui_tong_hop = '; '.join(parts)

    @api.depends('nhan_vien_nhan_ids', 'phong_ban_nhan_ids', 'khach_hang_nhan_ids', 'nguoi_nhan')
    def _compute_nguoi_nhan_tong_hop(self):
        for record in self:
            ...
            if record.nguoi_nhan:
                parts.append(record.nguoi_nhan)
